...
describe("hasResumePdf", () => {
  ...
  it("returns true when the public résumé PDF exists", async () => {
    const { hasResumePdf } = await import("@/lib/content/resume-pdf");
    expect(hasResumePdf()).toBe(true);
  });

  it("returns false when the public PDF is missing", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "resume-pdf-missing-"));
    fs.mkdirSync(path.join(tmpRoot, "public", "resume"), { recursive: true });
  ...
  it("returns true when a non-empty PDF file exists in a temp project root", async () => {
    ...
    fs.writeFileSync(
      ...
      "%PDF-1.4 stub content that is long enough",
    );
